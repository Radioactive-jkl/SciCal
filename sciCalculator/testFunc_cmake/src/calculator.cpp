...
string form_text(string in, string ans)
{
	...
	//clear " ", "="
	for (int i = in.find(" "); i <= in.length(); i = in.find(" "))
	{
		in.erase(i, 1);
	}
	if (in.empty())return "@";
	...
	if (in.length() == 1)
	{
		string check_0 = "0123456789";
		for (int i = 0; i < in.length(); i++)
		{
			if (check_0.find(in[i]) == check_0.npos)return "@";
		}
	}
	
	// check all valid strings
	/***************************
	 * ( ) s q r t i n c o a l *
	 * ^					   *
	 * + - * /				   *
	...
	for (int i = 0; i < in.length(); i++)
	{
		if (check_1.find(in[i]) == check_1.npos)return "@";
	}
	...
	for (int i = 0; i < in.length(); i++)
	{
		if (check_2.find(in[i]) == check_2.npos)return "@";
	}
	...
	{
		if (in.find(check_3[j]) != in.npos)return "@";
	}
